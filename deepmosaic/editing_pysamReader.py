...
class pysamReader():    
    def __init__(self, file_name, chrom, pos, ref=None, alt=None):
        if file_name.endswith('.bam'):
            self.samfile = pysam.AlignmentFile(file_name, "rb")
        elif file_name.endswith('.cram'):
            self.samfile = pysam.AlignmentFile(file_name, "rc")
        else:
    ...
    def downsample_to_max_depth(self):
        ...
        if self.reads_count > MAX_DP:
            #print("DOWN SAMPLE TO MAX reads count: " + str(self.reads_count))
            #print("Region  " + str(self.chrom) + ": " + str(self.pos+1))
            #print('\n')
